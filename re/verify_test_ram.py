#!/usr/bin/env python3
"""Verify terminal test-suite counters in a logical TilEm RAM dump."""
...
def main() -> int:
    ...
    latest_cell = next(
        item["addr"] for item in labels if item["name"] == "var-latest"
    )
    ...
    def u16(address: int) -> int:
        ...
        if offset < 0 or offset + 1 >= len(memory):
            raise ValueError(
                f"address outside logical RAM dump: {address:#06x}"
            )
    ...
    if args.before:
        ...
        for name in ("var-dp", "var-latest"):
            address = next(
                item["addr"] for item in labels if item["name"] == name
            )
