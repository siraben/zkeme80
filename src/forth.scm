...
(define forth-semantics-words
  `(
    ...
    ;; TOKEN has already measured the token.  Retain that length in B so
    ...
    ;; endpoint characters. TOKEN normally filters these before FIND-HEADER.
    (ld a b)
    (or a)
...
(define forth-control-words
  `(;; Runtime setup for DO.  The return stack layout remains
    ...
    (dw (parse-header dup @ latest ! dp ! clear-find-cache exit))
