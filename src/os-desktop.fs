...
VARIABLE OS-NOTICE
HERE 16 ALLOT CONSTANT OS-FILE-NAME
VARIABLE OS-FILE-NLEN
VARIABLE OS-FILE-TYPE
VARIABLE OS-FILE-VERSION
0 OS-CHOICE ! 0 OS-PAGE-NO ! 0 OS-PAGE-MODE !
1 OS-TASK-ID ! 0 OS-SERVICE-FIRST !
0 OS-FILE-NO ! 0 OS-FILE-OFFSET ! 0 OS-NOTICE !
0 OS-FILE-NLEN ! 0 OS-FILE-TYPE ! 65535 OS-FILE-VERSION !
: OS-KEY ( -- key ) KEY-EVENT ;
: OS-BACK? ( key -- flag ) DUP 15 = SWAP 50 = OR ;

\ Directory ordinals and FS buffers may change while OS-KEY yields. Keep the
\ displayed identity in view-owned memory, including its journal revision.
: OS-FILE-ID ( -- name len ) OS-FILE-NAME OS-FILE-NLEN @ ;
: OS-FILE-PIN ( -- )
  0 OS-FILE-NLEN ! 0 OS-FILE-TYPE !
  OS-FILE-NO @ FS-NTH IF
    OS-FILE-TYPE ! DROP DUP OS-FILE-NLEN ! OS-FILE-NAME SWAP CMOVE
    OS-FILE-ID FS-VERSION DROP OS-FILE-VERSION !
  ELSE 2DROP 2DROP THEN ;
: OS-FILE-CURRENT? ( -- flag )
  OS-FILE-ID FS-VERSION IF DROP 0 ELSE OS-FILE-VERSION @ = THEN ;
: OS-FILE-TEXT ( -- )
  OS-FILE-ID FS-GET IF DROP 2DROP 2 12 AT-XY ." Read error" ELSE
    OS-FILE-TYPE ! DUP IF DUP 1- 96 / 96 * ELSE 0 THEN
  ...
  THEN ;
: OS-FILE-LOAD ( -- )
  OS-FILE-CURRENT? 0= IF 5 OS-NOTICE ! EXIT THEN
  OS-FILE-TYPE @ 2 = IF
    OS-FILE-ID FS-LOAD IF 4 ELSE 3 THEN OS-NOTICE ! THEN ;
: OS-FILE-VIEW ( -- )
  0 OS-FILE-OFFSET ! 0 OS-NOTICE !
  BEGIN
    OS-FILE-CURRENT? 0= IF 5 OS-NOTICE ! THEN
    OS-FILE-ID FS-VERSION IF DROP 65535 THEN OS-FILE-VERSION !
    OS-FILE-ID OS-TITLE OS-FILE-TEXT
    OS-FOOT OS-NOTICE @ CASE
      3 OF ." Loaded / CLR back" ENDOF
      4 OF ." Load error / CLR back" ENDOF
      5 OF ." Changed / CLR back" ENDOF
      0 OF OS-FILE-TYPE @ 2 = IF ." <> page ENT load CLR"
        ELSE ." <> page CLR back" THEN ENDOF
    ENDCASE
...
: OS-FILE-DRAW ( -- )
  ...
  THEN
  OS-FOOT ." ^v ENT view CLR back" OS-FILE-PIN ;
: OS-FILES ( -- )
  BEGIN
    ...
    CASE
      ...
      9 OF OS-FILE-CURRENT? IF OS-FILE-VIEW THEN ENDOF
    ENDCASE
  AGAIN ;
