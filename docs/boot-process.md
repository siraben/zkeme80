...
```scheme
(ld a (#x0038))             ; IM 1 vector must contain real code
(cp #xff)
(ld hl (#x0056))
(ld bc #xa55a)
(sbc hl bc)
(jp z #x0053)               ; Jump to the installed OS entry.
```
...
`((label boot)
 (label shutdown)
 (di)
 (ld a 6)                    ; Slowest timer tick, independent mapping.
 (out (4) a)
 (ld a #x81)                 ; Window B -> RAM page 81.
 (out (7) a)
 (ld sp 0)
 (call sleep)                ; SLEEP selects IM 1.

 (label restart)
 (label reboot)
 (ld a 3)                    ; Extended flash-address bits.
 (out (#xe) a)
 (xor a)
 (out (#xf) a)
 (call unlock-flash)
 ;; Program execution-protection ports 22/23/25/26.
 (call lock-flash)
 (ld a 1)                    ; CPU clock: 15 MHz.
 (out (#x20) a)
 (ld a #b0001011)            ; ON + timer 1 + keep-power-in-HALT.
 (out (3) a)
 (ld hl #x8000)              ; Zero RAM from $8000 through $FFFF.
 (ld (hl) 0)
 (ld de #x8001)
 (ld bc #x7fff)
 (ldir)
 ;; Initialize the LCD through port $10, then splice in the Forth VM.
 ,@forth-asm
 (jp shutdown))
```
...
* When tracing under TilEm headless, seed the mapper replay with
  ...
  `debug/analyze_forth_trace.py` does this automatically.
